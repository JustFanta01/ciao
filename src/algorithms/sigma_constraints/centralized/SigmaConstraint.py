...
class SigmaConstraint(Algorithm):
    def __init__(self, problem: ConstrainedSigmaProblem):
        super().__init__(problem)
        self.lamda = None
        self.problem = cast(ConstrainedSigmaProblem, self.problem)
    ...
    def run(self, params: AlgorithmParams):
        ...
        N = self.problem.N
        d = self.problem.d
        m = self.problem.m # number of constraints per agent
        ...
        collector_projected_stationarity_dual = TrajectoryCollector("Projected_stationarity_traj_dual", K, (m,))
        ...
        def lagrangian():
            # $$ L(z,\lambda) =  \ell(z) + \lambda^\top(\sigma(z) - c) $$
        ...
        for k in range(K):
            ...
            # ora ha shape (N, d), compatibile con total_grad_f
            total_grad_L_in_x = total_grad_f + grad_lambda_times_constr
            assert total_grad_L_in_x.shape == (N, d)
            ...
            proj_residual_primal = (zz_k - zz_k_plus_1).reshape(N, d)

            
            # $$ \lambda^k - \Pi_{\mathbb{R}_m^+}\left(\lambda^k + \nabla_\lambda \mathcal L(x,\lambda)\right) $$            
            
            proj_dual = np.maximum(self.lamda + violation, 0.0)
            proj_residual_dual = self.lamda - proj_dual
