...
class CIAO(Algorithm):
    ...
    def run(self, params : AlgorithmParams):
        ...
        # $$ \sigma(z) \le c $$
        c = self.problem.global_matrices()
        ...
        for k in range(K):
            ...
            for i, agent_i in enumerate(self.problem.agents):
                ...
                # $$ a_i^{k+1} = \lambda_i^{k} + \beta (\sigma(z) - c) + n_i^k  $$
                ...
                # aa_k_plus_1[i] = agent_i["ll"] + beta * ((2 * ss_k_plus_1[i] - agent_i["ss"]) - c) + agent_i["nn"]
                # -------------------------------------------------------------
            ...
            # $$ \lambda^k - \Pi_{\mathbb{R}_m^+}\left(\lambda^k + \nabla_\lambda \mathcal L(x,\lambda)\right) - \frac{\theta}{\beta}\lambda^k$$
            
            proj_residual_dual = ll_k - np.maximum(ll_k + r - theta/beta*ll_k, 0.0)
