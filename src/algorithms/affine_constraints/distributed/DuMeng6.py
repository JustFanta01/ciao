...
class DuMeng6(Algorithm):
    ...
    def run(self, params : AlgorithmParams):
        ...
        collector_pp = TrajectoryCollector("pp", K, (N,m)) # proxy of $$\frac{1}{N} \sum_{j=1}^{N}\lambda_k$$
        ...
        for agent_i in self.problem.agents:
            ...
            # [ init pp ]
            agent_i["pp"] = agent_i["ll"] # $$ p_i^0 = \lambda_i^0$$
        ...
        for k in range(K):
            ...
            for i, agent_i in enumerate(self.problem.agents):
                ...
                # $$ a_i^{k+1} = p_i^k + \beta (B_i z_i^{k+1} - b_i)$$

                aa_k_plus_1[i] = agent_i["pp"] + beta * (B_i @ zz_k_plus_1[i] - b_i)
                
                # [ pp update ]
                
                # $$ p_i^{k+1} = \sum_{j=1}^{N} a_{ij} p_j^{k} + (a^{k+1} - a_i^{k}) $$
                
                pp_k = np.array([ag["pp"] for ag in self.problem.agents]) # (N,d)
                # ll_k = np.array([ag["ll"] for ag in self.problem.agents]) # (N,m)
                pp_consensus = pp_k.T @ adj_i.T
                pp_local_innovation = aa_k_plus_1[i] - agent_i["aa"]
                # pp_local_innovation = ll_k_plus_1[i] - agent_i["ll"]
                pp_k_plus_1[i] = pp_consensus + pp_local_innovation
            ...
            collector_pp.log(k, np.array([ag["pp"] for ag in self.problem.agents]))
