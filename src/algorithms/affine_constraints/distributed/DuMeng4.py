    ...
    def run(self, params : AlgorithmParams):
        ...
        for k in range(K):
            ...
            for i, agent_i in enumerate(self.problem.agents):
                ...
                # $$ s_i^{k+1} = \sum_{j=1}^{N} a_{ij} s_j^{k} + \phi_i(z_i^{k}) - \phi_i(z_i^{k-1}) $$
                
                ss_k = np.array([ag["ss"] for ag in self.problem.agents]) # (N,d)
                ss_local_innovation = agent_i.phi(agent_i["zz"]) - agent_i.phi(agent_i["zzm1"])
                ...
                nabla_2_k_minus_1 = agent_i.nabla_2(agent_i["zzm1"], agent_i["zzm1"]) 
                nabla_2_k = agent_i.nabla_2(agent_i["zz"], agent_i["ss"])
                vv_local_innovation = nabla_2_k - nabla_2_k_minus_1
                vv_k_plus_1[i] = vv_consensus + vv_local_innovation
                
                # [ zz update ]
                
                # $$ z_i^{k+1} = z_i^k - \alpha(\nabla_1 \ell_i(z_i^k, s_i^{k+1}) + \nabla \phi_i(z_i^k) v_i^{k+1} + B_i^T \lambda_i^k ) $$
                
                nabla_1 = agent_i.nabla_1(agent_i["zz"], ss_k_plus_1[i])
                nabla_phi = agent_i.nabla_phi(agent_i["zz"])
                grad_ell = nabla_1 + nabla_phi @ vv_k_plus_1[i]
            ...
            # NOTE: SECOND round of communication! in the same iteration...
            # - one for ss, vv
