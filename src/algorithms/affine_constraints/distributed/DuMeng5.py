...
class DuMeng5(Algorithm):
    ...
    def run(self, params : AlgorithmParams):
        ...
        collector_yy = TrajectoryCollector("yy", K, (N,m)) # aux
        ...
        for agent_i in self.problem.agents:
            ...
            # [ init yy ]
            agent_i["yy"] = np.zeros(m) # $$ n_i^0 = \mathbf{0} $$
        ...
        for k in range(K):
            ...
            for i, agent_i in enumerate(self.problem.agents):
                ...
                # $$ a_i^{k+1} = \lambda_i^{k} + \beta (B_i z_i^k - b_i) + \text{control\_input}  $$

                # aa_k_plus_1[i] = agent_i["ll"] + beta * (B_i @ (2 * zz_k_plus_1[i] - agent_i["zz"])) + agent_i["nn"]
                # aa_k_plus_1[i] = agent_i["ll"] + beta * (B_i @ agent_i["zz"]) + agent_i["nn"]
                
                # $$error_k = setpoint - variable = 0 - \mathcal L^2 \lambda_k$$
                ...
                # yy_consensus = yy_k.T @ rr_i.T
                ll_k = np.array([ag["ll"] for ag in self.problem.agents])       # (N, m)
                ll_consensus = ll_k.T @ rr_i.T
                
                # +--------------+
                # | PROPORTIONAL |
                # +--------------+
                setpoint = 0 # consensus
                error_k = setpoint - ll_k.T @ rr_i.T
                u1 = error_k

                # +--------------+
                # |   INTEGRAL   |
                # +--------------+
                # [ yy update ]: single integrator ($$\dot y = y + u$$) with P control input $$ u = err = (0 - \mathcal L^2 a_k) $$
                # NOTE: not the error_k on "ll" but on "aa" 
                yy_k_plus_1[i] = agent_i["yy"] + dt * error_k
                u2 = yy_k_plus_1[i]

                # +--------------+
                # |  DERIVATIVE  |
                # +--------------+
                u3 = (error_k - agent_i["error_k_minus_1"]) / dt
                agent_i["error_k_minus_1"] = error_k

                # PI controller for consensus:
                u = (K_P * u1) + (K_I * u2) + (K_D * u3)
                # aa_k_plus_1[i] = agent_i["aa"] + beta * (B_i @ zz_k_plus_1[i] - b_i) + u

                aa_consensus = aa_k.T @ adj_i.T
            ...
            collector_yy.log(k, np.array([ag["yy"] for ag in self.problem.agents]))
