...
class ComparisonBaseRunResultPlotter:
        ...
        if self._layout == "overlay":
            ...
            for r_idx, res in enumerate(self.results):
                for t_idx, task in enumerate(self._tasks):
                    ...
                    # opzionale: titoli di colonna / etichette di riga per orientarsi
                    if r_idx == 0:
                        ax.set_title(task.name)
                    if t_idx == 0:
                        ax.set_ylabel(res.algorithm_name)
    ...
    def _render_phase2d(self, ax, *,
        ...
        if result is None:
            for result in self.results:
                BaseRunResultPlotter._render_phase2d_from_result(ax, self.problem, result, show_cost_levels=show_cost_levels, level_grid=level_grid, level_count=level_count, title=title)
        else:
            BaseRunResultPlotter._render_phase2d_from_result(ax, self.problem, result, show_cost_levels=show_cost_levels, level_grid=level_grid, level_count=level_count, title=title)
